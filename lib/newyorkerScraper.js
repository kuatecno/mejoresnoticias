  ...
  for (const sitemapUrl of SITEMAP_URLS) {
    try {
      ...
      for (const e of entries) {
        if (e.loc && (
          e.loc.includes('/magazine/') || 
          e.loc.includes('/news/') || 
          e.loc.includes('/culture/') || 
          e.loc.includes('/podcast/') ||
          e.loc.includes('/puzzles-and-games-dept/') ||
          e.loc.includes('/humor/') ||
          e.loc.includes('/books/') ||
          e.loc.includes('/business/') ||
          e.loc.includes('/tech/') ||
          e.loc.includes('/politics/')
        )) {
          all.push(e);
        }
      }
...
function extractFromBody($) {
  // Try multiple selectors for New Yorker article content
  let bodyText = '';
  
  // Try main article content selectors
  const selectors = [
  ...
  for (const selector of selectors) {
    ...
    if (paragraphs.length > 0) {
      ...
      if (bodyText.length > 100) { // Only use if we got substantial content
        break;
      }
    }
  }
  ...
  // Check if we got meaningful content (not just paywall text)
...
async function scrapeArticle(url) {
  ...
  const fromBody = extractFromBody($);
